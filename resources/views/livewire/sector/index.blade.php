                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                         @foreach($datasets as $dataset)
                            <a href="{{ route('datasektoral.index', 'sector='.$dataset->id ) }}" class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                                <div class="p-4">
                                    <h3 class="text-lg font-medium text-gray-900">{{ $dataset->name }}</h3>
                                    <p class="text-sm text-gray-500">{{ $dataset->slug ?? 'Kode tidak tersedia' }}</p>
                                </div>
                            </a>
                        @endforeach
