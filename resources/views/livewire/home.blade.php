<div>
    {{-- ===== HERO ===== --}}
    <section class="bg-blue-900 text-white py-16 px-4">
        <div class="max-w-2xl mx-auto text-center">
            <span class="inline-block bg-yellow-500 text-blue-900 text-xs font-bold px-4 py-1 rounded-full mb-4 uppercase tracking-wider">
                <i class="fas fa-map-marker-alt mr-1"></i> Kabupaten Barito Utara
            </span>
            <h2 class="text-4xl font-bold mb-2">Satu Data <span class="text-yellow-400">Statistik Sektoral</span></h2>
            <p class="text-blue-300 mb-8 text-sm">Portal Data Terpadu Pemerintah Kabupaten Barito Utara</p>

            <div x-data="{ query: '' }" class="flex bg-white rounded-lg overflow-hidden shadow-lg">
                <span class="bg-yellow-500 text-blue-900 px-4 py-3 font-bold text-sm whitespace-nowrap">
                    <i class="fas fa-search mr-1"></i> Cari Data
                </span>
                <input
                    type="text"
                    x-model="query"
                    placeholder="Cari dataset, bidang, atau OPD..."
                    class="flex-1 px-4 py-3 text-gray-700 outline-none text-sm"
                />
                <button
                    @click="window.location.href = '{{ route('datasektoral.index') }}?search=' + encodeURIComponent(query)"
                    class="px-4 text-blue-900 hover:bg-gray-100 transition">
                    <i class="fas fa-arrow-right"></i>
                </button>
            </div>
        </div>
    </section>


    {{-- ===== STAT CARDS ===== --}}
    <section class="max-w-4xl mx-auto px-4 -mt-6 mb-10">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">

            <div class="bg-white rounded-xl shadow p-5 text-center">
                <div class="w-10 h-10 bg-blue-900 rounded-lg flex items-center justify-center mx-auto mb-3">
                    <i class="fas fa-database text-white text-sm"></i>
                </div>
                <p class="text-2xl font-bold text-blue-900">{{ number_format($totalDatasets) }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide mt-1">Dataset Tersedia</p>
            </div>

            <div class="bg-white rounded-xl shadow p-5 text-center">
                <div class="w-10 h-10 bg-blue-900 rounded-lg flex items-center justify-center mx-auto mb-3">
                    <i class="fas fa-building text-white text-sm"></i>
                </div>
                <p class="text-2xl font-bold text-blue-900">{{ $totalOrganizations }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide mt-1">OPD Tergabung</p>
            </div>

            <div class="bg-white rounded-xl shadow p-5 text-center">
                <div class="w-10 h-10 bg-blue-900 rounded-lg flex items-center justify-center mx-auto mb-3">
                    <i class="fas fa-layer-group text-white text-sm"></i>
                </div>
                <p class="text-2xl font-bold text-blue-900">{{ $totalCategories }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide mt-1">Bidang Sektoral</p>
            </div>

            <div class="bg-white rounded-xl shadow p-5 text-center">
                <div class="w-10 h-10 bg-blue-900 rounded-lg flex items-center justify-center mx-auto mb-3">
                    <i class="fas fa-eye text-white text-sm"></i>
                </div>
                <p class="text-2xl font-bold text-blue-900">{{ number_format($totalViews) }}</p>
                <p class="text-xs text-gray-500 uppercase tracking-wide mt-1">Total Views</p>
            </div>

        </div>
    </section>


    {{-- ===== CATEGORY CARDS ===== --}}
    <section class="max-w-6xl mx-auto px-4 mb-16">
        <div class="text-center mb-10">
            <p class="text-blue-900 text-xs font-bold uppercase tracking-widest mb-2">Jelajahi Data</p>
            <h3 class="text-3xl font-bold text-blue-900">Satu Data <span class="text-yellow-500">Statistik Sektoral</span></h3>
            <p class="text-gray-500 text-sm mt-2">Temukan data statistik berdasarkan bidang yang Anda butuhkan</p>
        </div>

        @php
            $colorMap = [
                'bg-blue-100'   => ['icon' => 'text-blue-500',   'border' => 'border-blue-500',   'label' => 'text-blue-500',   'arrow' => 'bg-blue-100 text-blue-500'],
                'bg-red-100'    => ['icon' => 'text-red-500',    'border' => 'border-red-500',    'label' => 'text-red-500',    'arrow' => 'bg-red-100 text-red-500'],
                'bg-green-100'  => ['icon' => 'text-green-500',  'border' => 'border-green-500',  'label' => 'text-green-500',  'arrow' => 'bg-green-100 text-green-500'],
                'bg-yellow-100' => ['icon' => 'text-yellow-600', 'border' => 'border-yellow-500', 'label' => 'text-yellow-600', 'arrow' => 'bg-yellow-100 text-yellow-600'],
                'bg-purple-100' => ['icon' => 'text-purple-500', 'border' => 'border-purple-500', 'label' => 'text-purple-500', 'arrow' => 'bg-purple-100 text-purple-500'],
                'bg-cyan-100'   => ['icon' => 'text-cyan-500',   'border' => 'border-cyan-500',   'label' => 'text-cyan-500',   'arrow' => 'bg-cyan-100 text-cyan-500'],
            ];
            $colorKeys = array_keys($colorMap);
        @endphp

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            @forelse($categories as $index => $cat)
                @php
                    $colorKey = $colorKeys[$index % count($colorKeys)];
                    $color    = $colorMap[$colorKey];
                    $icon     = $cat->icon ?? 'fas fa-layer-group'; // kolom icon di tabel categories (opsional)
                @endphp
                <a href="{{ route('datasektoral.index') }}?sector={{ $cat->id }}"
                   class="bg-white rounded-xl shadow p-6 border-t-4 {{ $color['border'] }} hover:shadow-lg hover:-translate-y-1 transition-all block">
                    <div class="w-12 h-12 {{ $colorKey }} rounded-xl flex items-center justify-center mb-4">
                        <i class="{{ $icon }} {{ $color['icon'] }} text-xl"></i>
                    </div>
                    <p class="text-xs {{ $color['label'] }} font-bold uppercase tracking-wider mb-1">Bidang</p>
                    <h4 class="text-lg font-bold mb-2">{{ $cat->name }}</h4>
                    <p class="text-gray-500 text-sm mb-4">{{ $cat->description ?? '-' }}</p>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-400">
                            <i class="fas fa-database mr-1 {{ $color['icon'] }}"></i>
                            {{ $cat->dataset_uploads_count }} dataset
                        </span>
                        <span class="w-8 h-8 {{ $color['arrow'] }} rounded-full flex items-center justify-center text-xs">
                            <i class="fas fa-arrow-right"></i>
                        </span>
                    </div>
                </a>
            @empty
                <div class="col-span-3 text-center text-gray-400 py-10">
                    <i class="fas fa-inbox text-4xl mb-3 block"></i>
                    <p>Belum ada kategori</p>
                </div>
            @endforelse
        </div>
    </section>


    {{-- ===== TRENDING ===== --}}
    <section class="max-w-6xl mx-auto px-4 mb-16">
        <div class="bg-blue-900 rounded-2xl p-8">
            <div class="flex items-end justify-between mb-6 flex-wrap gap-4">
                <div>
                    <h3 class="text-2xl font-bold text-white">Dataset <span class="text-yellow-400">Trending</span></h3>
                    <p class="text-blue-300 text-sm mt-1">Dataset populer berdasarkan jumlah view</p>
                </div>
                <a href="{{ route('datasektoral.index') }}"
                   class="text-yellow-400 text-sm font-semibold border border-yellow-400 border-opacity-50 px-4 py-2 rounded-full hover:bg-yellow-400 hover:bg-opacity-10 transition">
                    Lihat semua <i class="fas fa-arrow-right ml-1"></i>
                </a>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @forelse($trending as $item)
                    <div class="flex items-center justify-between bg-white bg-opacity-10 border border-white border-opacity-10 rounded-xl px-4 py-3 hover:bg-opacity-20 transition cursor-pointer">
                        <p class="text-white text-xs flex-1 mr-3">{{ Str::upper($item->title) }}</p>
                        <span class="bg-yellow-400 bg-opacity-20 text-yellow-400 text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap">
                            {{ number_format($item->view_count ?? 0) }} views
                        </span>
                    </div>
                @empty
                    <div class="col-span-2 text-center text-blue-300 py-6">
                        <p>Belum ada dataset trending</p>
                    </div>
                @endforelse
            </div>
        </div>
    </section>
</div>