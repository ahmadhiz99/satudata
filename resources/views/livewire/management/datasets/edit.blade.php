    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <i class="fas fa-pen-to-square mr-2"></i> Edit Dataset
            </h2>
                ...
                <form wire:submit.prevent="save" class="p-6 space-y-6">
                    ...
                    <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">
                            <i class="fas fa-file-excel mr-1 text-green-600"></i> File Saat Ini
                        </p>
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-800 truncate">{{ $upload->file_name }}</p>
                                <p class="text-xs text-gray-400">{{ $upload->file_size_formatted }} · {{ $upload->created_at?->format('d M Y') ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        @if($excel_file)
                            <div class="mt-2 text-sm text-green-600">
                                <i class="fas fa-check-circle mr-1"></i> {{ $excel_file->getClientOriginalName() }}
                            </div>
                        @endif
                        ...
                        @error('excel_file') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        <p class="text-xs text-gray-400 mt-2">
                            <i class="fas fa-info-circle mr-1"></i> Format .xlsx atau .xls, maksimal 10MB.
                        </p>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <label for="extract_to_db" class="text-sm font-medium text-gray-700 cursor-pointer">
                                Ekstrak Ke Database
                            </label>
                        </div>
                            ...
                            <div class="ml-6 space-y-4 border-l-2 border-blue-200 pl-4">
                                ...
                                <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-4">
                                    <p class="text-xs text-blue-600 font-medium">
                                        <i class="fas fa-table mr-1"></i> Konfigurasi Posisi Data Excel
                                    </p>
                                    <p class="text-xs text-blue-500">
                                        Tentukan posisi awal data. Baris header diambil otomatis dari baris sebelum baris mulai.
                                    </p>
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" wire:model="start_row" min="2"
                                                   ...
                                                   placeholder="5">
                                            @error('start_row') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                        ...
                                        <div>
                                            ...
                                            <input type="text" wire:model="start_col" maxlength="3"
                                                   class="w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-red-500 focus:ring-red-500 uppercase @error('start_col') border-red-500 @enderror"
                                                   placeholder="A">
                                            @error('start_col') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                    ...
                                    <div class="flex items-start gap-2 text-xs text-blue-700 bg-blue-100 rounded p-2">
                                        ...
                                        <span>
                                            ...
                                            header dibaca dari baris <strong>{{ max(1, $start_row - 1) }}</strong>.
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                    <div class="flex items-start gap-3">
                                        <input type="checkbox"
                                               wire:model.live="re_extract"
                                               id="re_extract"
                                               class="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300 rounded mt-0.5">
                                        <div>
                                            <label for="re_extract" class="text-sm font-medium text-amber-800 cursor-pointer">
                                                Ekstrak Ulang Data
                                            </label>
                                            <p class="text-xs text-amber-600 mt-0.5">
                                                <i class="fas fa-triangle-exclamation mr-1"></i>
                                                Seluruh data lama akan dihapus dan diganti dengan data dari file Excel (lama atau baru).
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-3">
                                    <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                                        <i class="fas fa-eye mr-1"></i> Tipe Visualisasi
                                    </p>

                                    {{-- Tabel --}}
                                    <div class="flex items-center gap-3 py-2.5 px-3 bg-white border rounded-lg transition-colors
                                        {{ $visualize_table ? 'border-blue-300 bg-blue-50' : 'border-gray-200' }}">
                                        <input type="checkbox" wire:model.live="visualize_table" id="visualize_table"
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                        <label for="visualize_table" class="flex items-center gap-2 text-sm cursor-pointer">
                                            <i class="fas fa-table w-4 {{ $visualize_table ? 'text-blue-500' : 'text-gray-400' }}"></i>
                                            <span class="font-medium {{ $visualize_table ? 'text-gray-700' : 'text-gray-500' }}">Tabel</span>
                                        </label>
                                    </div>

                                    {{-- Grafik --}}
                                    <div class="flex items-center gap-3 py-2.5 px-3 bg-white border rounded-lg transition-colors
                                        {{ $visualize_chart ? 'border-blue-300 bg-blue-50' : 'border-gray-200' }}">
                                        <input type="checkbox" wire:model.live="visualize_chart" id="visualize_chart"
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                        <label for="visualize_chart" class="flex items-center gap-2 text-sm cursor-pointer">
                                            <i class="fas fa-chart-bar w-4 {{ $visualize_chart ? 'text-blue-500' : 'text-gray-400' }}"></i>
                                            <span class="font-medium {{ $visualize_chart ? 'text-gray-700' : 'text-gray-500' }}">Grafik</span>
                                        </label>
                                    </div>

                                    {{-- Peta --}}
                                    <div class="flex items-center gap-3 py-2.5 px-3 bg-white border rounded-lg transition-colors
                                        {{ $visualize_map ? 'border-blue-300 bg-blue-50' : 'border-gray-200' }}">
                                        <input type="checkbox" wire:model.live="visualize_map" id="visualize_map"
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                        <label for="visualize_map" class="flex items-center gap-2 text-sm cursor-pointer">
                                            <i class="fas fa-map w-4 {{ $visualize_map ? 'text-blue-500' : 'text-gray-400' }}"></i>
                                            <span class="font-medium {{ $visualize_map ? 'text-gray-700' : 'text-gray-500' }}">Peta</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-4">
                                    <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                                        <i class="fas fa-info-circle mr-1"></i> Metadata Dataset
                                    </p>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" wire:model="start_year"
